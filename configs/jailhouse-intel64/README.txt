...
for the NuttX JAILHOUSE i486 port.
...
JAILHOUSE is a generic and open source machine emulator and virtualizer.  Here are
some links (which will probably be mostly outdated by the time your read this):

  Home Page:     http://wiki.jailhouse.org/Main_Page
  Downloads:     http://wiki.jailhouse.org/Download
  Documentation: http://wiki.jailhouse.org/Manual
  Usage:         jailhouse -nographic -kernel nuttx.elf

Building JAILHOUSE
-------------

  tar zxf jailhouse-0.14.0.tar.gz
  cd jailhouse-0.14.0
...
---------------------
  ...
  Workaround:
    ...
    None known.  It does not seem possible to build JAILHOUSE using the Cygwin gcc.
    I tried editing configure.  Removing the following line will allow JAILHOUSE to
    configure:

      JAILHOUSE_CFLAGS="-mno-cygwin $JAILHOUSE_CFLAGS"

    However, it then fails later during the compilation phase.

  Recommendation:

    1. Google for "jailhouse windows download" and download some pre-built JAILHOUSE
       binaries.  I found 0.14.0 here: http://dietpc.org/windows/jailhouse/, or
    2. Try building JAILHOUSE with MingGW (I understand that this is difficult).

  NOTE: As of this writing, I have not been successful getting ANY pre-built
  version of JAILHOUSE to work successfully with WinXP.  The same binaries work
...
------------
  ...
  In the top-level NuttX directory:
    ...
    jailhouse -cpu 486 -m 2 -kernel nuttx.elf -nographic
  ...
  option does not work under Cygwin.  For simple testing under Cygwin, I use
    ...
    jailhouse -cpu 486 -m 2 -kernel nuttx.elf -serial file:test.txt
...
==========
  ...
  files).  Those cannot be used with JAILHOUSE.
...
--------------------------
  ...
  generate the i486 ELF files needed by JAILHOUSE.  The NuttX buildroot package will
...
----------------------
  ...
  1. You must have already configured Nuttx in <some-dir>/nuttx.
     ...
     tools/configure.sh jailhouse-i486/<sub-dir>
...
--------------------------
  ...
  1. Each Qemu-i486 Web Server configuration is maintained in a sub-directory
     and can be selected as follow:
       ...
       tools/configure.sh jailhouse-i486/<subdir>
